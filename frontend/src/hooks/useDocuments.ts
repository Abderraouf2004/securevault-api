...
export function useDocuments() {
  return useQuery({
    queryKey: KEY,
    queryFn: async () => (await apiClient.get<ApiSuccess<DocumentDTO[]>>("/documents")).data.data,
  });
}
...
export function useCreateDocument() {
  ...
  return useMutation({
    mutationFn: async (input: { title: string; description?: string; file: File }) => {
      ...
      return (await apiClient.post<ApiSuccess<DocumentDTO>>("/documents", form)).data.data;
    },
...
export function useUpdateDocument() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...body }: { id: string; title?: string; description?: string }) =>
      (await apiClient.put<ApiSuccess<DocumentDTO>>(`/documents/${id}`, body)).data.data,
    onSuccess: () => qc.invalidateQueries({ queryKey: KEY }),
  });
}
