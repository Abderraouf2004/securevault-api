...
export default function SecretsPage() {
  ...
  //   { mode: "create" } | { mode: "edit"; secret: SecretListDTO } | null
  // >(null);
  const [modalState, setModalState] = useState<
  { mode: "create" } | { mode: "edit"; secret: SecretListDTO } | null
>(null);
  ...
  const handleDelete = async (secret: SecretListDTO) => {
    if (!confirm(`Delete "${secret.name}"? This can't be undone.`)) {
      return;
    }
      ...
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            API keys, passwords and tokens — encrypted with AES-256-GCM.
          </p>
        </div>
...
function SecretRow({
  ...
  const fetchSecret = async (): Promise<string | null> => {
    try {
      ...
      const response = await apiClient.get<ApiSuccess<SecretDTO>>(
        `/secrets/${secret.id}`,
      );
  ...
  return (
    <li className="flex items-center justify-between gap-4 rounded-md border border-line bg-surface px-4 py-3.5 shadow-card">
      ...
      <div className="flex shrink-0 items-center gap-1 text-muted">
        <span className="mr-2 hidden text-xs sm:inline">
...
function SecretModal({
  ...
  state:
    | { mode: "create" }
    | { mode: "edit"; secret: SecretListDTO }
    | null;
  onClose: () => void;
}) {
  const create = useCreateSecret();
  ...
  /*
   * When editing a secret, the list only contains metadata.
   * We therefore retrieve the decrypted value explicitly.
   */
  const loadSecretForEdit = async (secret: SecretListDTO) => {
    try {
  ...
  /*
   * Load the secret value when the edit modal is opened.
   */
  const handleModalState = async () => {
    ...
    await loadSecretForEdit(editing);
  };

  /*
   * The modal is controlled by the parent, so detect when
   * an edit modal is opened.
   */
  // const previousSecretId = editing?.id;

  // useState(() => {
  //   if (previousSecretId) {
  //     void handleModalState();
  //   }
  // });
  useEffect(() => {
  if (!editing) {
  ...
  void loadSecretForEdit(editing);
}, [editing?.id]);
